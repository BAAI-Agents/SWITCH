...
SOURCE_FOLDER_NAME = "环境与气候控制-问卷"
SOURCE_JSON_NAME = "SWITCHEcosystem.json"
OUTPUT_NAME = "hf_innovative_qa_v2_multiform_environment_v1"
SOURCE_FILE = SOURCE_JSON_NAME
SCHEMA_VERSION = "switch-hf-innovative-qa-v2-multiform-environment-v1"
SCENARIO_FAMILY = "environment_climate_control"
LABELS = ["A", "B", "C", "D"]
LABELS5 = ["A", "B", "C", "D", "E"]
TMP_CLIP_DIR = Path.cwd() / ".codex_tmp" / "environment_qa_clip_tmp"
...
FINAL_STATE_VIDEO2TXT_END_GAP_FRAMES = 60
RECOVERY_ABNORMAL_IMG2IMG_QUERY_GAP_FRAMES = 30
VERIFICATION_ACTION_SAME_CATEGORY_MIN_TEXT_DISTANCE = 0.62
...
class FrameSignal:
    ...
    @property
    def origin_qa_id(self) -> str:
        return f"{self.data_id}_{self.source_label}_{self.frame:06d}"
...
class SegmentSignal:
    ...
    @property
    def origin_qa_id(self) -> str:
        return f"{self.data_id}_{self.source_label}_{self.start:06d}_{self.end:06d}"
...
class VideoRecord:
    ...
    ui_change_segments: list[SegmentSignal] = field(default_factory=list)
    physical_world_changes: list[SegmentSignal] = field(default_factory=list)
    physical_world_states: list[FrameSignal] = field(default_factory=list)
    ui_states: list[FrameSignal] = field(default_factory=list)
    abnormal_signals: list[FrameSignal] = field(default_factory=list)
    ...
    @property
    def wrong_actions(self) -> list[ActionSegment]:
        return [action for action in self.actions if "wrong" in normalize_text(action.action_type)]
...
def parse_records(source_json: Path, source_dir: Path) -> list[VideoRecord]:
    ...
    for item in data:
        ...
        final_state_frame = next((start for start, end, _ in scalar_labels.get("is_final_state", []) if start == end), 0)
        has_final_state = final_state_frame > 0
        ...
        ui_change_segments = [
            SegmentSignal(data_id=data_id, video_name=video_name, start=start, end=end, text=text, source_label="ui_change")
            for start, end, text in scalar_labels.get("ui_change", [])
            if start != end and text
        ]
        physical_world_changes = [
            SegmentSignal(
                ...
                text=text,
                source_label="physical_world_change",
            )
            for start, end, text in scalar_labels.get("physical_world_change", [])
            if start != end and text
        ]
        physical_world_states = [
            FrameSignal(
                data_id=data_id,
                video_name=video_name,
                frame=start,
                text=text,
                source_label="physical_world_state",
            )
            for start, end, text in scalar_labels.get("physical_world_state", [])
            if start == end and text
        ]
        ui_states = [
            FrameSignal(data_id=data_id, video_name=video_name, frame=start, text=text, source_label="ui_state")
            for start, end, text in scalar_labels.get("ui_state", [])
            if start == end and text
        ]
        abnormal_signals = [
            FrameSignal(data_id=data_id, video_name=video_name, frame=start, text=text, source_label="Abnormal")
            for start, end, text in scalar_labels.get("Abnormal", [])
            if start == end and text
        ]
        ...
        records.append(
            VideoRecord(
                ...
                ui_change_segments=ui_change_segments,
                physical_world_changes=physical_world_changes,
                physical_world_states=physical_world_states,
                ui_states=ui_states,
...
def select_final_state_video2txt_query_range(record: VideoRecord) -> tuple[int, int, str]:
    end = clamp_frame(record.final_state_frame - FINAL_STATE_VIDEO2TXT_END_GAP_FRAMES, record.total_frames)
    if end >= record.final_state_frame:
        end = clamp_frame(record.final_state_frame - 1, record.total_frames)
    start = 1
    if end < start:
        end = start
    return start, end, f"prefix_until_final_state_minus_{FINAL_STATE_VIDEO2TXT_END_GAP_FRAMES}f"
...
def fallback_text_candidates() -> list[dict]:
    return [
        text_candidate("distractor::fallback::unrelated_climate_setting", "Check an unrelated climate-control setting", "fallback_text"),
        text_candidate("distractor::fallback::different_ac_control", "Press a different air-conditioner control", "fallback_text"),
        text_candidate("distractor::fallback::look_away", "Look away from the climate-control panel", "fallback_text"),
        text_candidate("distractor::fallback::wait_without_change", "Wait without changing the air-conditioner state", "fallback_text"),
        text_candidate("distractor::fallback::wrong_temperature", "Confirm a temperature that does not match the target", "fallback_text"),
    ]
...
def build_verification_action_img2txt_text_options(
    ...
    for candidate in same_category_pool:
        if normalize_text(candidate["text"]) == correct_key:
            continue
        distance = text_difference_score(correct["text"], candidate["text"])
        if distance < VERIFICATION_ACTION_SAME_CATEGORY_MIN_TEXT_DISTANCE:
            continue
        enriched = dict(candidate, source_type="same_category_far_verification_action")
        enriched["text_distance_to_correct"] = distance
        same_candidates.append(enriched)
    same_other_video = [candidate for candidate in same_candidates if candidate.get("data_id") != correct_action.data_id]
    same_rank_pool = same_other_video or same_candidates
    same_rank_pool = sorted(
        unique_action_candidates(same_rank_pool),
        key=lambda candidate: (
            candidate.get("text_distance_to_correct", text_difference_score(correct["text"], candidate["text"])),
    ...
    option_category_names = [option.get("category_name", current_category_name) for option in options]
    strategy_meta = {
        "distractor_strategy": "category_aware_one_far_same_category_two_different_category",
        "same_category_min_text_distance": VERIFICATION_ACTION_SAME_CATEGORY_MIN_TEXT_DISTANCE,
        "option_text_distances_to_correct": [
            round(text_difference_score(correct["text"], option["text"]), 4) if normalize_text(option["text"]) != correct_key else 0.0
            for option in options
        ],
        "same_category_distractor_count": sum(1 for x in option_source_types if x == "same_category_far_verification_action"),
...
def base_item(
    ...
    return {
        ...
        "slice_tags": ["environment_climate_control", "state_aligned", "extra_signal_aware"],
        "question_zh": "",
        ...
        "notes": "Generated from SWITCHEcosystem Label Studio annotations. Environment-specific branches use UI, physical-world, and abnormal signals when available.",
    }
...
def state_pool(records: list[VideoRecord]) -> list[tuple[str, str, Path, int, str, dict]]:
    ...
    for record in records:
        ...
        for signal in record.ui_states + record.physical_world_states:
            pool.append(
                (
                    signal.origin_qa_id,
                    clean_answer(signal.text),
                    record.video_path,
                    signal.frame,
                    f"{signal.source_label}_frame",
                    {"start": signal.frame, "end": signal.frame},
                )
            )
    return pool
...
def ui_verification_candidates(record: VideoRecord, current_category: str) -> list[dict]:
        ...
        candidates.append(
            {
                ...
                "source_type": "ui_change_state_frame",
                "source_span": {"start": ui.frame, "end": ui.frame},
                "verification_evidence_type": "ui_signal",
                "record": record,
                "category_name": current_category,
                "data_id": record.data_id,
            }
        )
    for ui_state in record.ui_states:
        candidates.append(
            {
                "origin_qa_id": ui_state.origin_qa_id,
                "answer": clean_answer(ui_state.text),
                "video_path": record.video_path,
                "video_name": record.video_name,
                "frame": ui_state.frame,
                "source_type": "ui_state_frame",
                "source_span": {"start": ui_state.frame, "end": ui_state.frame},
                "verification_evidence_type": "ui_signal",
                "record": record,
                "category_name": current_category,
                "data_id": record.data_id,
...
def physical_verification_candidates(record: VideoRecord, current_category: str) -> list[dict]:
    candidates: list[dict] = []
    for state in record.physical_world_states:
        candidates.append(
            {
                "origin_qa_id": state.origin_qa_id,
                "answer": clean_answer(state.text),
                "video_path": record.video_path,
                "video_name": record.video_name,
                "frame": state.frame,
                "source_type": "physical_world_state_frame",
                "source_span": {"start": state.frame, "end": state.frame},
                "verification_evidence_type": "physical_world_state",
                "record": record,
                "category_name": current_category,
                "data_id": record.data_id,
            }
        )
    for change in record.physical_world_changes:
        candidates.append(
            {
                "origin_qa_id": f"{change.origin_qa_id}_mid",
                "answer": clean_answer(change.text),
                "video_path": record.video_path,
                "video_name": record.video_name,
                "frame": change.mid_frame,
                "source_type": "physical_world_change_mid_frame",
                "source_span": {"start": change.start, "end": change.end},
                "verification_evidence_type": "physical_world_state",
                "record": record,
                "category_name": current_category,
                "data_id": record.data_id,
...
def verification_multiselect_targets(records: list[VideoRecord]) -> list[dict]:
    targets: list[dict] = []
    current_category = category_key(SOURCE_FOLDER_NAME)
    for record in records:
        ui_candidates = ui_verification_candidates(record, current_category)
        physical_candidates = physical_verification_candidates(record, current_category)
        if not ui_candidates or not physical_candidates:
            continue
        ui_target = sorted(ui_candidates, key=lambda candidate: candidate["frame"])[0]
        physical_target = sorted(
            physical_candidates,
            key=lambda candidate: (
                0 if candidate["source_type"] == "physical_world_state_frame" else 1,
                candidate["frame"],
            ),
        )[0]
        targets.append(
            {
                ...
                "record": record,
                "correct_targets": [ui_target, physical_target],
            }
        )
    return targets
...
def select_verification_video2txt_query_range(
    record: VideoRecord, action: ActionSegment
) -> tuple[int, int, str, str | None]:
    previous = previous_action_segment(record, action)
    if previous is not None:
        ...
        return start, end, "prefix_until_previous_action_end", previous.origin_qa_id
    start, end, rule = select_first_action_pre_video_range(record, action)
    return start, end, rule, None
        ...
        for action in actions:
            ...
            if task_family == "verification_action":
                if current_form == "img2txt":
                    query_stem = (
                        f'Based on this frame, what verification action is the operator performing '
                        f'to confirm "{record.overall_requirement}"? Answer with a short action phrase.'
                    )
                    prompt_variant = "verify_action_current_frame__action_phrase"
                    semantic_anchor = "verification_action_performed"
                else:
            ...
            if current_form == "video2txt":
                if task_family == "action":
                    ...
                    query_start, query_end, query_rule, query_from_action = select_verification_video2txt_query_range(record, action)
                    video_out = form / "videos" / f"{origin}_query_video.mp4"
                    write_clip(record.video_path, query_start, query_end, video_out, record.fps)
                    item["query_video_path"] = rel(video_out, form)
                    item["query_source_range"] = {"start_frame": query_start, "end_frame": query_end}
                    item["query_source_rule"] = query_rule
                    item["query_source_action_qa_id"] = query_from_action
                    item["future_gap_frames"] = action.start - query_end
                    item["strict_future_pass"] = query_end < action.start
                else:
                    video_out = form / "videos" / f"{record.data_id}.mp4"
                    ensure_video_copy(record.video_path, video_out)
                    item["query_video_path"] = rel(video_out, form)
            ...
            if current_form == "video2txt":
                ...
                if task_family in {"action", "verification_action"}:
                    ...
                    if task_family == "verification_action":
                        open_item["future_gap_frames"] = item["future_gap_frames"]
                        open_item["strict_future_pass"] = item["strict_future_pass"]
            else:
...
def build_final_state_text_forms(records: list[VideoRecord], output_root: Path) -> None:
    records = [record for record in records if record.has_final_state]
    current_category_name = category_key(output_root.parent.name)
    answer_pool = [
        final_state_text_candidate(
    ...
    for current_form in ["video2txt", "img2txt"]:
        ...
        for record in records:
            ...
            if current_form == "video2txt":
                query_stem = (
                    f'Watch the video up to 60 frames before the annotated final state. '
                    f'What visible outcome should confirm that "{record.overall_requirement}" succeeds? '
                    f'Answer with a short phrase.'
                )
                prompt_variant = "final_state_pre_final_context_60f__mcq"
                query_temporal_role = "context_until_60_frames_before_final_state"
            else:
                query_stem = f'What visible outcome confirms that "{record.overall_requirement}" has succeeded? Answer with a short phrase.'
                prompt_variant = "final_state_visible_outcome__mcq"
                query_temporal_role = "annotated_final_state_frame"
            option_category_names = None
            distractor_strategy_meta = {}
            ...
            if current_form == "video2txt":
                query_start, query_end, query_rule = select_final_state_video2txt_query_range(record)
                video_out = form / "videos" / f"{origin}_query_video.mp4"
                write_clip(record.video_path, query_start, query_end, video_out, record.fps)
                item["query_video_path"] = rel(video_out, form)
                item["query_source_range"] = {"start_frame": query_start, "end_frame": query_end}
                item["query_source_rule"] = query_rule
                item["query_temporal_role"] = query_temporal_role
                item["future_gap_frames"] = record.final_state_frame - query_end
                item["strict_future_pass"] = item["future_gap_frames"] >= FINAL_STATE_VIDEO2TXT_END_GAP_FRAMES
            else:
                img_out = form / "imgs" / f"{origin}_query_img.jpg"
            ...
            open_item = base_item(
                ...
                "final_state",
                "L3",
                prompt_variant.replace("__mcq", "__open"),
                "keyword",
                "visible_outcome",
            ...
            if current_form == "video2txt":
                ...
                open_item["query_source_range"] = item["query_source_range"]
                open_item["query_source_rule"] = item["query_source_rule"]
                open_item["query_temporal_role"] = item["query_temporal_role"]
                open_item["future_gap_frames"] = item["future_gap_frames"]
                open_item["strict_future_pass"] = item["strict_future_pass"]
            else:
                open_item["query_img_path"] = item["query_img_path"]
...
def build_verification_state_text_forms(records: list[VideoRecord], output_root: Path) -> None:
    ...
    for current_form in ["video2txt", "img2txt"]:
        ...
        for target in targets:
            ...
            ui_target = next(t for t in correct_targets if t["verification_evidence_type"] == "ui_signal")
            physical_target = next(t for t in correct_targets if t["verification_evidence_type"] == "physical_world_state")
            if current_form == "img2txt":
            ...
            item.update(
                {
                    ...
                    "query_temporal_role": "command_context_before_verification_states",
...
def verification_frame_signal_pool(records: list[VideoRecord], include_abnormal: bool = False) -> list[dict]:
    pool: list[dict] = []
    current_category = category_key(SOURCE_FOLDER_NAME)
    for record in records:
        for ui in record.ui_changes:
        ...
        for signal in record.ui_states + record.physical_world_states:
            pool.append(
                {
                    "origin_qa_id": signal.origin_qa_id,
                    "answer": clean_answer(signal.text),
                    "video_path": record.video_path,
                    "video_name": record.video_name,
                    "frame": signal.frame,
                    "source_type": f"{signal.source_label}_frame",
                    "source_span": {"start": signal.frame, "end": signal.frame},
                    "record": record,
                    "category_name": current_category,
                    "data_id": record.data_id,
                }
        ...
        if record.has_final_state:
            pool.append(
                {
                    "origin_qa_id": f"{record.data_id}_is_final_state",
                    "answer": clean_answer(record.overall_verification),
                    "video_path": record.video_path,
                    "video_name": record.video_name,
                    "frame": record.final_state_frame,
        ...
        if include_abnormal:
            for signal in record.abnormal_signals:
                pool.append(
                    {
                        "origin_qa_id": signal.origin_qa_id,
                        "answer": clean_answer(signal.text),
...
def verification_segment_signal_pool(records: list[VideoRecord]) -> list[dict]:
    pool: list[dict] = []
    current_category = category_key(SOURCE_FOLDER_NAME)
    for record in records:
        for signal in record.ui_change_segments + record.physical_world_changes:
            pool.append(
                {
                    "origin_qa_id": signal.origin_qa_id,
                    "answer": clean_answer(signal.text),
                    ...
                    "start": signal.start,
                    "end": signal.end,
                    "source_type": f"{signal.source_label}_segment",
                    "source_span": {"start": signal.start, "end": signal.end},
                    "record": record,
                    "category_name": current_category,
                    "data_id": record.data_id,
                }
...
def choose_frame_signal_options(correct: dict, pool: list[dict], origin: str, k: int = 4) -> list[dict]:
    rng = random.Random(stable_int(origin + "::frame_signal_options"))
    same_video = [
        dict(candidate, source_type=f"same_video_{candidate['source_type']}")
        for candidate in pool
        if candidate["origin_qa_id"] != correct["origin_qa_id"]
        and candidate.get("record") is not None
        and candidate["record"].data_id == correct["record"].data_id
        and abs(candidate["frame"] - correct["frame"]) >= 20
    ]
    other = [
        dict(candidate, source_type=f"other_video_{candidate['source_type']}")
        for candidate in pool
        if candidate["origin_qa_id"] != correct["origin_qa_id"]
        and candidate.get("record") is not None
        and candidate["record"].data_id != correct["record"].data_id
    ]
    rng.shuffle(same_video)
    rng.shuffle(other)
    options = [dict(correct, source_type=f"correct_{correct['source_type']}")]
    for candidate in same_video[:1] + other:
        if candidate["origin_qa_id"] in {option["origin_qa_id"] for option in options}:
            continue
        options.append(candidate)
        if len(options) == k:
            break
...
def choose_segment_signal_options(correct: dict, pool: list[dict], origin: str, k: int = 4) -> list[dict]:
    rng = random.Random(stable_int(origin + "::segment_signal_options"))
    same_video = [
        dict(candidate, source_type=f"same_video_{candidate['source_type']}")
        for candidate in pool
        if candidate["origin_qa_id"] != correct["origin_qa_id"]
        and candidate.get("record") is not None
        and candidate["record"].data_id == correct["record"].data_id
        and not spans_overlap(candidate["start"], candidate["end"], correct["start"], correct["end"])
    ]
    other = [
        dict(candidate, source_type=f"other_video_{candidate['source_type']}")
        for candidate in pool
...
def closest_wrong_action(record: VideoRecord, abnormal: FrameSignal) -> ActionSegment | None:
    wrong_actions = record.wrong_actions
    if wrong_actions:
        return min(wrong_actions, key=lambda action: abs(action.end - abnormal.frame))
    before = [action for action in record.actions if action.end <= abnormal.frame]
    if before:
        return sorted(before, key=lambda action: action.end)[-1]
    return record.last_action


def inferred_fix_action(record: VideoRecord, abnormal: FrameSignal, wrong_action: ActionSegment | None) -> str:
    text = clean_answer(abnormal.text)
    lower = normalize_text(text)
    if "should" in lower:
        return text
    if wrong_action is not None and wrong_action.requirement:
        return f"Redo the step correctly: {wrong_action.requirement}."
    return f"Use the correct control to satisfy the task: {record.overall_requirement}."


def abnormal_text_options(correct: str, pool: list[str], origin: str) -> tuple[str, dict[str, str], list[str], list[str]]:
    candidates = [text_candidate(f"{origin}_correct", correct, "correct_abnormal_annotation")]
    candidates.extend(text_candidate(f"{origin}_distractor_{idx}", value, "abnormal_text_pool") for idx, value in enumerate(pool) if value != correct)
    gt, option_map, _, option_origin_ids, option_source_types = build_text_options(correct, candidates, origin, correct_origin_qa_id=f"{origin}_correct")
    return gt, option_map, option_origin_ids, option_source_types


def build_recovery_abnormal_forms(records: list[VideoRecord], output_root: Path) -> None:
    abnormal_pairs = [
        (record, abnormal, closest_wrong_action(record, abnormal))
        for record in records
        for abnormal in record.abnormal_signals
    ]
    all_abnormal_texts = [abnormal.text for _, abnormal, _ in abnormal_pairs]
    all_wrong_actions = [action.description for _, _, action in abnormal_pairs if action is not None]
    all_fix_texts = [inferred_fix_action(record, abnormal, action) for record, abnormal, action in abnormal_pairs]

    form = form_dir(output_root, "recovery_abnormal", "img2txt")
    vqa: list[dict] = []
    openqa: list[dict] = []
    for record, abnormal, wrong_action in abnormal_pairs:
        img_out = form / "imgs" / f"{abnormal.origin_qa_id}_query_img.jpg"
        query_frame = clamp_frame(abnormal.frame - RECOVERY_ABNORMAL_IMG2IMG_QUERY_GAP_FRAMES, record.total_frames)
        write_frame(record.video_path, query_frame, img_out)
        question_specs = [
            (
                "abnormal_identification",
                "What abnormal state is visible in this frame?",
                abnormal.text,
                all_abnormal_texts,
                "abnormal_visible_state",
            ),
            (
                "recovery_next_action",
                f'The task was "{record.overall_requirement}", but this frame shows a failure state. What should be done next to recover?',
                inferred_fix_action(record, abnormal, wrong_action),
                all_fix_texts,
                "next_recovery_action",
            ),
            (
                "counterfactual_verification",
                f'Why does this frame not verify success for "{record.overall_requirement}"?',
                abnormal.text,
                all_abnormal_texts,
                "not_success_reason",
            ),
        ]
        for subtype, query_stem, answer, pool, semantic_anchor in question_specs:
            origin = f"{abnormal.origin_qa_id}_{subtype}"
            gt, option_map, option_origin_ids, option_source_types = abnormal_text_options(answer, pool, origin)
            item = base_item(
                origin,
                with_options(query_stem, option_map),
                gt,
                ...
                subtype,
                "L4",
                f"environment_{subtype}__img2txt_mcq",
                "keyword",
                semantic_anchor,
                {"type": "choice", "labels": LABELS, "choice_modality": "text"},
                answer,
                {"start": abnormal.frame, "end": abnormal.frame},
            )
            item.update(
                {
                    "query_img_path": rel(img_out, form),
                    "query_source_frame": query_frame,
                    "query_source_rule": f"abnormal_frame_minus_{RECOVERY_ABNORMAL_IMG2IMG_QUERY_GAP_FRAMES}f",
                    "future_gap_frames": abnormal.frame - query_frame,
                    "strict_future_pass": abnormal.frame > query_frame,
                    "abnormal_frame": abnormal.frame,
                    "abnormal_signal": abnormal.text,
                    "wrong_action": wrong_action.description if wrong_action else "",
                    "inferred_fix_action": inferred_fix_action(record, abnormal, wrong_action),
            ...
            open_item = base_item(
                ...
                subtype,
                "L4",
                f"environment_{subtype}__img2txt_open",
                "keyword",
                semantic_anchor,
                {"type": "keyword", "value": semantic_anchor},
                answer,
                {"start": abnormal.frame, "end": abnormal.frame},
            )
            open_item.update(
            ...
            openqa.append(open_item)
    write_dataset(output_root, "recovery_abnormal", "img2txt", vqa, openqa)

    form = form_dir(output_root, "recovery_abnormal", "video2txt")
    vqa = []
    openqa = []
    for record, abnormal, wrong_action in abnormal_pairs:
        if wrong_action is None:
            continue
        query_start = clamp_frame(max(1, wrong_action.start - 15), record.total_frames)
        query_end = clamp_frame(abnormal.frame, record.total_frames)
        video_out = form / "videos" / f"{abnormal.origin_qa_id}_wrong_to_abnormal.mp4"
        write_clip(record.video_path, query_start, query_end, video_out, record.fps)

        origin = f"{abnormal.origin_qa_id}_wrong_action_location"
        gt, option_map, option_origin_ids, option_source_types = abnormal_text_options(
            wrong_action.description,
            all_wrong_actions,
            origin,
        )
        query_stem = "Watch the clip ending at the abnormal state. Which action most likely caused the failure?"
        item = base_item(
            origin,
            with_options(query_stem, option_map),
            gt,
            "recovery_abnormal",
            "wrong_action_localization",
            "L4",
            "environment_wrong_action_location__video2txt_mcq",
            "keyword",
            "wrong_action_cause",
            {"type": "choice", "labels": LABELS, "choice_modality": "text"},
            wrong_action.description,
        ...
        item.update(
            {
                "query_video_path": rel(video_out, form),
                "query_source_range": {"start_frame": query_start, "end_frame": query_end},
                "wrong_action_range": {"start_frame": wrong_action.start, "end_frame": wrong_action.end},
                "abnormal_frame": abnormal.frame,
                "abnormal_signal": abnormal.text,
                "correct_answer": wrong_action.description,
            }
        )
        add_option_text_fields(item, option_map, option_origin_ids, option_source_types)
        vqa.append(item)
        ...
        chain = {
            "wrong_action": wrong_action.description,
            "post_wrong_signal": abnormal.text,
            "fix_action": inferred_fix_action(record, abnormal, wrong_action),
            "expected_success_signal": record.overall_verification,
        }
        chain_origin = f"{abnormal.origin_qa_id}_recovery_chain"
        chain_query = "Summarize the failure and recovery path. Reply with wrong_action, post_wrong_signal, fix_action, and expected_success_signal."
        open_item = base_item(
            chain_origin,
            chain_query,
            ...
            "recovery_chain",
            "L5",
            "environment_recovery_chain_with_post_wrong_signal__video2txt_open",
            "structured_short_answer",
            "wrong_action_fix_chain",
            {
                "type": "structured_short_answer",
                "fields": ["wrong_action", "post_wrong_signal", "fix_action", "expected_success_signal"],
            },
            json.dumps(chain, ensure_ascii=False),
            {"start": query_start, "end": query_end},
        )
        ...
        openqa.append(open_item)
    write_dataset(output_root, "recovery_abnormal", "video2txt", vqa, openqa)

    form = form_dir(output_root, "recovery_abnormal", "img2img")
    vqa = []
    frame_pool = verification_frame_signal_pool(records, include_abnormal=True)
    for record, abnormal, wrong_action in abnormal_pairs:
        correct = {
            "origin_qa_id": abnormal.origin_qa_id,
            "answer": abnormal.text,
            "video_path": record.video_path,
            "video_name": record.video_name,
            "frame": abnormal.frame,
            "source_type": "Abnormal_frame",
            "source_span": {"start": abnormal.frame, "end": abnormal.frame},
            "record": record,
            "category_name": category_key(SOURCE_FOLDER_NAME),
            "data_id": record.data_id,
        }
        query_img = form / "imgs" / f"{abnormal.origin_qa_id}_query_img.jpg"
        query_frame = clamp_frame(abnormal.frame - RECOVERY_ABNORMAL_IMG2IMG_QUERY_GAP_FRAMES, record.total_frames)
        write_frame(record.video_path, query_frame, query_img)
        query = f'Which image option shows the abnormal state rather than a successful state for "{record.overall_requirement}"?'
        item = base_item(
            f"{abnormal.origin_qa_id}_abnormal_vs_success_img2img",
            query,
            "",
            "recovery_abnormal",
            "abnormal_vs_success_state",
            "L4",
            "environment_abnormal_vs_success__img2img",
            "choice",
            "abnormal_state_discrimination",
            {"type": "choice", "labels": LABELS, "choice_modality": "image"},
            abnormal.text,
            {"start": abnormal.frame, "end": abnormal.frame},
        )
        item.update(
            {
                "query_img_path": rel(query_img, form),
                "query_source_frame": query_frame,
                "query_source_rule": f"abnormal_frame_minus_{RECOVERY_ABNORMAL_IMG2IMG_QUERY_GAP_FRAMES}f",
                "future_gap_frames": abnormal.frame - query_frame,
                "strict_future_pass": abnormal.frame > query_frame,
                "abnormal_frame": abnormal.frame,
                "wrong_action": wrong_action.description if wrong_action else "",
                "correct_answer": abnormal.text,
            }
        )
        options = choose_frame_signal_options(correct, frame_pool, item["origin_qa_id"])
        option_paths = []
        option_frames = []
        option_origin_ids = []
        option_source_types = []
        for label, option in zip(LABELS, options):
            option_img = form / "imgs" / f"option_img_{abnormal.origin_qa_id}_{label}.jpg"
            write_frame(option["video_path"], option["frame"], option_img)
            option_paths.append(rel(option_img, form))
            option_frames.append(option["frame"])
            option_origin_ids.append(option["origin_qa_id"])
            option_source_types.append(option["source_type"])
            if option["origin_qa_id"] == abnormal.origin_qa_id:
                item["GT"] = label
        item.update(
            {
                "option_imgs_path": option_paths,
        ...
        vqa.append(item)
    write_dataset(output_root, "recovery_abnormal", "img2img", vqa, [])
...
def write_environment_metadata(output_root: Path, records: list[VideoRecord]) -> None:
    manifest_path = output_root / "dataset_manifest.json"
    if not manifest_path.exists():
        return
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["schema_version"] = SCHEMA_VERSION
    manifest["source_json"] = SOURCE_JSON_NAME
    manifest["scenario_family"] = SCENARIO_FAMILY
    manifest["annotation_summary"] = {
        "videos": len(records),
        "action_segments": sum(len(record.actions) for record in records),
        "verification_action_segments": sum(len([action for action in record.actions if action.is_verification]) for record in records),
        "wrong_action_segments": sum(len(record.wrong_actions) for record in records),
        "ui_change_frames": sum(len(record.ui_changes) for record in records),
        "ui_change_segments": sum(len(record.ui_change_segments) for record in records),
        "ui_state_frames": sum(len(record.ui_states) for record in records),
        "physical_world_change_segments": sum(len(record.physical_world_changes) for record in records),
        "physical_world_state_frames": sum(len(record.physical_world_states) for record in records),
        "is_final_state_frames": sum(1 for record in records if record.has_final_state),
        "abnormal_frames": sum(len(record.abnormal_signals) for record in records),
        "verification_state_multiselect_samples": len(verification_multiselect_targets(records)),
        "recovery_abnormal_samples": sum(len(record.abnormal_signals) for record in records),
    }
    manifest["generation_notes"] = [
        "Environment/climate QA is generated from SWITCHEcosystem Label Studio timeline annotations.",
        "Core task/action/final_state forms reuse the lighting multiform layout for path compatibility.",
        "Explicit is_final_state frames are used for whole-video final-state GT; records without is_final_state are skipped in final_state visual/text forms.",
        "final_state/video2txt query clips are truncated at is_final_state - 60 frames to avoid leaking the annotated final state.",
        "verification_state uses UI-side signals plus physical-world state signals when both are available.",
        "physical_world_state is preferred as the physical verification state; physical_world_change can provide a mid-frame state fallback inside verification_state.",
        "Standalone verification-evidence forms are not generated; verification evidence is consolidated under verification_state.",
        "Abnormal frames are kept out of ordinary success verification and are used only in recovery_abnormal tasks.",
        "recovery_abnormal includes abnormal identification, wrong-action localization, inferred next recovery action, abnormal-vs-success image choice, recovery chain, and counterfactual verification variants.",
        "Abnormal annotations do not contain confirmed correction segments; fix_action is inferred from abnormal text or the paired wrong-action requirement.",
    ]
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")

    readme = [
        "# Environment QA v2 Multiform v1",
        "",
        "Generated from `SWITCHEcosystem.json` using the SWITCH v2 multiform layout with environment-specific signal handling.",
        "",
        "## Key Decisions",
        "",
        "- Standalone verification-evidence forms are not generated; verification evidence is consolidated under `verification_state`.",
        "- `physical_world_state` is treated as a single-frame physical verification state.",
        "- `physical_world_change` can provide a mid-frame physical-state fallback inside `verification_state`.",
        "- `ui_state` and single-frame `ui_change` are treated as UI-side verification states.",
        "- `Abnormal` is treated as a single-frame failure signal and only feeds `recovery_abnormal` tasks.",
        "- `recovery_abnormal` contains abnormal identification, wrong-action localization, abnormal-after-failure recovery, abnormal-vs-success choice, recovery-chain OpenQA, and counterfactual verification.",
        "- Final-state forms use explicit `is_final_state`; videos without that marker are skipped for final-state GT instead of using arbitrary last frames.",
        "- `final_state/video2txt` query videos end exactly 60 frames before `is_final_state`, so the GT final-state frame is not visible in the input.",
        "",
        "## Counts",
        "",
    ]
    for key, value in manifest["annotation_summary"].items():
